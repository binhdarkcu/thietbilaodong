...
function mo_openid_gdpr()
{
    ...
    <form id="gdpr" name="gdpr" method="post" action="">
        ...
        <div class="mo_openid_table_layout">
            <label class=" mo_openid_note_style" style="font-size:small;padding:22px;">If GDPR check is enabled, users will be asked to give consent before using Social Login. Users who will not give consent will not be able to log in. This setting stands true only when users are registering using Social Login. This will not interfere with users registering through the regular WordPress.<br><br>(Click <a target="_blank" href="<?php echo add_query_arg( array('tab' => 'privacy_policy'), $_SERVER['REQUEST_URI'] ); ?>"> here </a> to read miniOrange Social Login Privacy Policy. Please update your website's privacy policy accordingly and enter the URL to your privacy policy below.)</label>
            <br/>
            <label class="mo_openid_checkbox_container">Take consent from users
            ...
            <label style="font-size: 12px">Enter the Consent message: </label><br/>
            ...
            <br><br>
            <label style="font-size: 12px"> Enter the text to be displayed for the Privacy Policy URL :</label><br/>
            ...
            <br><br>
            <label style="font-size: 12px">Enter Privacy Policy URL: </label><br/>
            ...
            <br/><br/><b><input type="submit" <?php echo $disable?> name="submit" value="Save" style="width:150px;background-color:#0867b2;color:white;box-shadow:none;text-shadow: none;"  class="button button-primary button-large" /></b>
        </div>
    </form>
    <script>
        //to set heading name
        jQuery('#mo_openid_page_heading').text('GDPR Settings');
