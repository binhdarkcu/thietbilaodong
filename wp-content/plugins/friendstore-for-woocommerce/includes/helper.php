...
if (!function_exists('convert_to_grams')) {
    function convert_to_grams(float $weight = 0, $reverse = false) {
        $weight_unit = get_option('woocommerce_weight_unit');
        if ($weight_unit == 'kg') {
            return $reverse ? $weight / 1000 : $weight * 1000;
        } else if ($weight_unit == 'lbs') {
            return $reverse ? $weight / 453.59237 : $weight * 453.59237;
        } else if ($weight_unit == 'oz') {
            return $reverse ? $weight / 28.3495231 : $weight * 28.3495231;
        }
        return $weight;
    }
}
...
if (!function_exists('get_parcel_data')) {
    ...
    function get_parcel_data($products, $density = '') {
        ...
        $total_weight += (float)get_vfoption('packaging_weight', 'fsw_shipping', '1.00');
...
if (!function_exists('track_order_status')) {
    function track_order_status(int $order_ID = 0) {
